function CategoryIcon({ iconKey, isActive }) {
    ...
    switch (iconKey) {
      case "hotBeverages":
        return (
          <svg viewBox="0 0 48 48" aria-hidden="true" className="h-7 w-7 sm:h-8 sm:w-8 lg:h-9 lg:w-9">
  ...
  export default function MenuCategoryTabs({
    ...
    return (
      <div className="relative w-full">
        <div className="overflow-x-auto pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden [scroll-snap-type:x_mandatory] [-webkit-overflow-scrolling:touch]">
          <div className="flex min-w-max gap-3 px-1 sm:gap-4 lg:hidden">
            {categories.map((category) => {
              ...
              return (
                <button
                  ...
                  className={`[scroll-snap-align:start] flex min-w-[92px] shrink-0 flex-col items-center justify-center rounded-[22px] border px-3 py-3 text-center transition ${
                    isActive
                      ? "border-[#7a3d14] bg-[linear-gradient(180deg,#7a3d14_0%,#5d2c0e_100%)] text-[#fffaf4] shadow-[0_10px_22px_rgba(91,44,14,0.18)]"
                      : "border-[#eaded2] bg-[#fffaf5] text-[#2d1a10]"
                  }`}
                >
                  <div className="flex h-9 items-center justify-center">
                    <CategoryIcon iconKey={category.iconKey} isActive={isActive} />
                  </div>
  
                  <span
                    className={`mt-2 text-[0.72rem] font-bold uppercase tracking-[0.05em] ${
                      isActive ? "text-[#fffaf4]" : "text-[#3d2a20]"
                    }`}
          ...
          <div className="hidden lg:block">
            <div className="grid w-full grid-cols-7 rounded-[28px] border border-[#eaded2] bg-[#fffaf5] p-2 shadow-[0_8px_24px_rgba(86,50,28,0.06)]">
                ...
                return (
                  <div
                    ...
                    className={`relative min-w-0 ${
                      !isLast
                        ? "after:absolute after:right-0 after:top-1/2 after:h-[58%] after:w-px after:-translate-y-1/2 after:bg-[#eaded2]"
                        : ""
                    }`}
                  >
                    <button
                      type="button"
                      onClick={() => onCategoryChange(category.slug)}
                      className={`flex h-full w-full flex-col items-center justify-center rounded-[22px] px-4 py-5 text-center transition duration-200 ${
                        isActive
                    ...
                    >
                      ...
                      <span
                        className={`mt-3 text-[0.98rem] font-bold uppercase tracking-[0.04em] ${
